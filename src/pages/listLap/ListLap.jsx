...
export default function ListLap({ data, setData }) {
  ...
  const handleSelect = (id, newRights) => {
    setData(
      data.map(item =>
        item.id === id ? { ...item, rights: newRights } : item
      )
    );
    setOpenId(null);
  };
  ...
  const filteredData = data.filter(item =>
    [item.name, item.number, item.DeviceID]
      ...
      <div className="control__header-actions">
        <div className="control__search-container">
          <input
            ...
            placeholder="Поиск по имени, ID или Device ID"
            value={searchTerm}
                  ...
                  <div className="dropdown">
                    <div
                      className={`dropdown__header ${
                        openId === item.id ? 'dropdown__header--open' : ''
                      }`}
                    ...
                    {openId === item.id && (
                      <div className="dropdown__list">
                        <div
                          ...
                          onClick={() => handleSelect(item.id, "Учебный")}
                        >
                          Учебный
                        </div>
                        <div
                          className="dropdown__item"
                          onClick={() => handleSelect(item.id, "Root")}
                        >
                          Root
                        </div>
                      </div>
